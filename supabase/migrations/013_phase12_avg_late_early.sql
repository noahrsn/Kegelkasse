-- ============================================================================
-- Kegelkasse — Phase 12: Nachzügler/Frühgeher = Durchschnitt statt Strafe
-- ----------------------------------------------------------------------------
-- Änderungen an der Genehmigungs-Buchung:
--   * Neuer Teilnehmer-Status „geht früher" (is_early_leave) analog zu Nachzügler
--     (is_late). Beide sind im Frontend frei umschaltbar (reversibel).
--   * Nachzügler bekommen KEINE Verspätungsstrafe mehr, sondern den Durchschnitt
--     der voll anwesenden Mitglieder. Frühgeher ebenso (Schnitt des Restabends).
--   * Der Durchschnitt = Σ Strafen der voll Anwesenden ÷ Anzahl voll Anwesende
--     (voll anwesend = nicht Gast, nicht Nachzügler, nicht Frühgeher).
--   * Die Abwesenden-Durchschnittsstrafe (charge_absent_avg) nutzt denselben Wert.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- save_session — is_early_leave je Teilnehmer mitschreiben (Signatur unverändert).
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    INSERT INTO session_participants (session_id, user_id, guest_name, is_guest, is_late, is_early_leave)
    VALUES (
      sid,
...
-- ----------------------------------------------------------------------------
-- approve_session — Durchschnitt für Nachzügler/Frühgeher (+ optional Abwesende).
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Schuld je voll anwesendem Mitglied aus der Summe seiner Strafen
  -- (voll anwesend = nicht Gast, nicht Nachzügler, nicht Frühgeher).
  ...
  -- Durchschnitt der voll Anwesenden berechnen.
  ...
  -- Nachzügler + Frühgeher bekommen den Durchschnitt (keine Verspätungsstrafe).
  IF v_avg > 0 THEN
    INSERT INTO debts (user_id, group_id, type, amount, description, session_id, due_date, created_by)
    SELECT p.user_id,
           s.group_id,
           ...
           v_avg,
           'Durchschnitt '
             || CASE WHEN p.is_late THEN '(Nachzügler) '
                     WHEN p.is_early_leave THEN '(früher gegangen) '
                     ELSE '' END
    ...
    WHERE p.session_id = s.id
      AND p.is_guest = false
      AND p.user_id IS NOT NULL
      AND (p.is_late OR p.is_early_leave);
    GET DIAGNOSTICS avg_n = ROW_COUNT;
  END IF;

  -- Optional: Abwesende mit demselben Durchschnitt belasten.
  IF s.charge_absent_avg AND v_avg > 0 THEN
    INSERT INTO debts (user_id, group_id, type, amount, description, session_id, due_date, created_by)
    SELECT a.user_id,
           s.group_id,
           'penalty',
